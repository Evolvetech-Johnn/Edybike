...
import { deals, formatPrice } from '../../data/categoryProducts';
import { useCart } from '../../context/CartContext';
import { ProductWithDiscount } from '../../types';
import { FaStar, FaShoppingCart, FaEye, FaTags, FaPercent } from 'react-icons/fa';

const CategoryDeals: FC = () => {
  const [products] = useState<ProductWithDiscount[]>(deals);
      ...
      <div style={{
        ...
        <div className="container">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '1rem', marginBottom: '1rem' }}>
            <FaTags style={{ fontSize: '2.5rem' }} />
            <h1 style={{ fontSize: 'clamp(2rem, 5vw, 3rem)', margin: 0 }}>Ofertas Especiais</h1>
          </div>
          <p style={{ fontSize: '1.2rem', opacity: 0.9 }}>
            Descontos imperdíveis de até 31% OFF!
          </p>
        </div>
      </div>
          ...
          {products.map((product) => (
            <div key={product.id} className="card fade-in">
              <div className="card-img-container" style={{ height: '250px', position: 'relative' }}>
                ...
                <div className="tag" style={{ position: 'absolute', top: '1rem', left: '1rem', background: 'linear-gradient(135deg, #f59e0b, #d97706)', color: 'white', fontSize: '1rem', fontWeight: '800' }}>
